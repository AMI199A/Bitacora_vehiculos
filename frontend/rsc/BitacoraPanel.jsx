...
export const BitacoraPanel = () => {
    ...
    const cargarSemana = async () => {
        const res = await fetch('http://localhost:3000/api/comisiones/semanal?fecha_inicio=2026-08-10&fecha_fin=2026-08-14');
    ...
    const descargarPDF = (id) => {
        window.open(`http://localhost:3000/api/comisiones/${id}/pdf`, '_blank');
    ...
    const ejecutarCierreAnual = async () => {
        ...
        const response = await fetch('http://localhost:3000/api/admin/cierre-anual', {
            method: 'POST',
